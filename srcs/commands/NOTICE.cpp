...
NOTICE::NOTICE(void): Command("NOTICE", 0, true) {};
NOTICE::~NOTICE(void) {};

void NOTICE::execute(Server* server, Client* client, IRCMessage message, std::vector<execReturnData>& execReturn) {
                ...
                if (!destinationChannel)
                    ...
                    for (std::vector<int>::iterator it = joinedChannelFds.begin(); it != joinedChannelFds.end(); it++) {
                        ...
                        if (channelClient && channelClient->getFd() != client->getFd()) {
                            ...
                            outData += RPL_NOTICE(client->getFullUserId(), "#" + destination, msg) + CRLF;
                        };
                    };
                    if (bot)
